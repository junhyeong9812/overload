...
Overload는 Java 25의 **Virtual Threads**를 활용한 경량 HTTP 부하 테스트 CLI 도구입니다.

`wrk`, `hey`, `k6`처럼 커맨드라인에서 간편하게 사용할 수 있으며, Virtual Threads의 경량성 덕분에 적은 리소스로 대규모 동시 요청을 생성할 수 있습니다.
...
# 설치 (PATH에 추가)
./gradlew installDist
export PATH=$PATH:$(pwd)/overload-cli/build/install/overload/bin
```
...
Options:
  ...
  -d, --duration <SEC>      테스트 지속 시간 (요청 수 대신 사용)
  -H, --header <HEADER>     HTTP 헤더 (여러 번 사용 가능)
  -d, --data <BODY>         요청 본문
  -t, --timeout <MS>        요청 타임아웃 (기본: 5000ms)
...
System.out.println("p99: " + result.percentiles().p99() + "ms");
```
...
```
overload/
├── overload-core/          # 핵심 엔진 (순수 Java 라이브러리)
│   └── src/main/java/
...
│           └── output/     # 출력 포매터
│
├── overload-web/           # 웹 대시보드 (선택적, Spring Boot)
│   └── src/main/java/
│       └── io.github.junhyeong9812.overload.web/
│
├── docs/
...
| 모듈 | 기술 |
|------|------|
| **overload-core** | Java 25, Virtual Threads, JDK HttpClient |
| **overload-cli** | picocli, jansi (색상 출력) |
| **overload-web** | Spring Boot 4.0.0, Thymeleaf (선택적) |
| **Build** | Gradle (멀티 모듈) |
...
| 도구 | 언어 | 특징 |
|------|------|------|
| **Overload** | Java (Virtual Threads) | JVM 환경, 경량, 라이브러리로 사용 가능 |
...
| 문서 | 설명 |
|------|------|
| [아키텍처 철학](docs/architecture/README.md) | 멀티 모듈 + 헥사고날 구조 설명 |
